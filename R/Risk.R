...
ignitions <- function(weather, smoulder = 24, Extinction = 0.15){
  ...
  strike <- weather %>% arrange(desc(cgStrikes))
...
frameSpread <- function(base.params, weather, a, igLoc = -1, t = 1, hourStep = 3, Extinction = 0.15, fReach = 0, fEdge = 1, fireArea = 100,
                        wHeading = 0, firelineW = 1, slopeM = 6.7, slopeSD = 4.9, slopeLength = 391, mRiver = 0.38, lRiver = 0.06, 
                        Test = 80, sensitive = 1, girdleH, woodDensity = 1000, barkDensity = 300, bark = 0.04, comBark = 700, 
                        resBark = 0,  phloem = 0.01, moisture = 0.6,  bMoisture = 0.5, FPC = 0.5, distance = 2,  trail = 1000,  var = 10,
                        Altitude = 200, vAir500 = 2, necT = 60, surfDecl = 10, minR = 0.001, targSp = "c") {
  
  # Create random database name for parallel processing
  aTxt <- case_when(fEdge == 1 ~ "Head",
                    fEdge == -1 ~ "Tail",
                    TRUE ~ "Flank")
  dbH <- paste(aTxt, hourStep, "_", a, "_", round(runif(1, min = 1, max = 10000000),3), ".db", sep = "")
  Reach <- vector()
  Reach[1] <- fReach
  travDist <- vector()
  travDist[1] <- 0
  ...
  while (remTime > 0) {
    ...
    if (asp < Extinction) {
      if (asp %in% aspList) {
        #GET VALUES FOR ASPLIST AND BEHAVIOUR PARAMETERS
        ...
        TBL <- base.params %>%
          ffm_set_site_param("windSpeed", weather$Wind[t]*(0.0361*wHeading*slopeM-0.0005*wHeading*slopeM^2+1)*fEdge, "km/h") %>%
          ...
          ffm_set_site_param("slope", slopeLand*wHeading*fEdge) # Does not account for cross-wind spreading both up and down slope
        ffm_run(TBL, db.path = dbH, db.recreate = recreate)
        res<-ffm_db_load(dbH)
        R[landStep] <- max(res$ROS$ros)*1000
        # Find spotting distance
        Rs <- spotFire(flameHeight = max(res$FlameSummaries$flameHeight),slope = (slopeLand*wHeading*fEdge),FPC,
                       windExposure = wHeading * landscapeLoc[landStep-1],vAir = weather$Wind[t]*(0.0361*wHeading*slopeM-0.0005*wHeading*slopeM^2+1)*fEdge,
                       vAir500,fireArea)  
        # Find next reach
        ...
        # Calculate fire effects and add to output table
        ...
        if (is.null(girdleH)) {
          ...
          for (Height in girdleH) {
            if (R[landStep] <= minR) {
              ...
              stem <- frame::girdle(Surf = runs, Plant = IP, Height = Height, woodDensity = woodDensity, phloem = phloem, 
                                    barkDensity = barkDensity, bark = bark, comBark = comBark, resBark = resBark, Altitude = Altitude,  
                                    moisture = moisture,  bMoisture = bMoisture,  distance = distance,  trail = trail,  var = var,  diameter = litterDepth,
                                    RH = weather$RH[t], Pressure = weather$MSLP[t], startTemp = weather$Temp[t], necT = necT, surfDecl = surfDecl, updateProgress = NULL)
    ...
    if (R[landStep] == 0) {
      ...
      # Moisture of landform across the barrier
      ...
      # Likelihood of crossing by spotting
      spotI <- as.numeric(runif(1) <= min(1,max(0,-2.75 + 20.95*runif(1, min = 0, max = 0.5) - 32*dfmc))) # Replace with spotting function
...
burnPrint <- function(Flora, Structure, default.species.params, a, weather, smoulder = 24, Extinction = 0.15, hourStep = 3, tArea = 10,
                      ...
                      distance = 0.1,  trail = 10,  var = 10, Altitude = 200, necT = 60, surfDecl = 10, minR = 0.001, targSp = "a") {
  ...
  while(Stop == 0) {
    ...
    if (stopNS < smoulder) {
      fEdge <- case_when(wEvent$Direction[t] == 0 ~ 1,  #1~Head, 0~Flank, -1~Tail
                         ...
                         TRUE ~ 0)
      edgeA <- frameSpread(base.params = TBL, weather = wEvent, a, igLoc = igLoc, t = t, hourStep = hourStep, vAir500, FPC, fireArea,
                           slopeM = slopeM, slopeSD = slopeSD, slopeLength = slopeLength, mRiver = mRiver, lRiver = lRiver,
                           wHeading = wHeading, firelineW = W[t], fReach = fReachA, fEdge = fEdge, Extinction = Extinction, 
                           Test = Test, sensitive = sensitive, girdleH = girdleH, woodDensity = woodDensity, phloem = phloem, moisture = moisture,  
                           barkDensity = barkDensity, bark = bark, comBark = comBark, resBark = resBark,  bMoisture = bMoisture, 
                           distance = distance,  trail = trail,  var = var, Altitude = Altitude, necT = necT, surfDecl = surfDecl, minR = minR, targSp = targSp)
      tabA <- edgeA[[1]] %>%
    ...
    if (stopWE < smoulder) {
      fEdge <- case_when(wEvent$Direction[t] == 270 ~ 1,  #1~Head, 0~Flank, -1~Tail
                         ...
                         TRUE ~ 0)
      edgeB <- frameSpread(base.params = TBL, weather = wEvent, a, igLoc = igLoc, t = t, hourStep = hourStep, vAir500, FPC, fireArea,
                           slopeM = slopeM, slopeSD = slopeSD, slopeLength = slopeLength, mRiver = mRiver, lRiver = lRiver,
                           wHeading = wHeading, firelineW = L[t], fReach = fReachB, fEdge = fEdge, Extinction = Extinction, 
                           Test = Test, sensitive = sensitive, girdleH = girdleH, woodDensity = woodDensity, phloem = phloem, moisture = moisture,  
                           barkDensity = barkDensity, bark = bark, comBark = comBark, resBark = resBark,  bMoisture = bMoisture, 
                           distance = distance,  trail = trail,  var = var, Altitude = Altitude, necT = necT, surfDecl = surfDecl, minR = minR, targSp = targSp) 
      tabA <- edgeB[[1]] %>%
    ...
    if (stopSN < smoulder) {
      fEdge <- case_when(wEvent$Direction[t] == 180 ~ 1,  #1~Head, 0~Flank, -1~Tail
                         ...
                         TRUE ~ 0)
      edgeC <- frameSpread(base.params = TBL, weather = wEvent, a, igLoc = igLoc, t = t, hourStep = hourStep, vAir500, FPC, fireArea, 
                           slopeM = slopeM, slopeSD = slopeSD, slopeLength = slopeLength, mRiver = mRiver, lRiver = lRiver,
                           wHeading = wHeading, firelineW = W[t], fReach = fReachC, fEdge = fEdge, Extinction = Extinction, 
                           Test = Test, sensitive = sensitive, girdleH = girdleH, woodDensity = woodDensity, phloem = phloem, moisture = moisture,  
                           barkDensity = barkDensity, bark = bark, comBark = comBark, resBark = resBark,  bMoisture = bMoisture, 
                           distance = distance,  trail = trail,  var = var, Altitude = Altitude, necT = necT, surfDecl = surfDecl, minR = minR, targSp = targSp) 
      tabA <- edgeC[[1]] %>%
...
frameRisk <- function(Flora, Structure, default.species.params, a = a, weather, lightning = 0.05,
...
parRisk <- function(a) {
  ...
  res <- frame::frameRisk(Flora = FloraA, Structure = StructureA, default.species.params, a, weather, lightning,
                          ...
                          distance,  trail,  var, Altitude, necT, surfDecl, minR, targSp)
...
riskDynamics <- function(fireDat, tr, clim, lightning = 0.05, 
  ...
  parallel::clusterExport(cl,varlist=c('Flora', 'Structure', 'default.species.params', 'clim', 'm', 'rholitter',
                                       ...
                                       'smoulder', 'Extinction', 'hourStep', 'studyArea', 'vAir500',
